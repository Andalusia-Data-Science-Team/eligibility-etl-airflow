...
file_handler = TimedRotatingFileHandler(
    LOG_DIR / "debug.log", when="midnight", interval=1, backupCount=7, encoding="utf-8"
)
file_handler.setLevel(logging.DEBUG)

# Console handler for INFO-level feedback only
...
try:
    ...
    read_passcode = db_configs["read"] 

    with open(BASE_DIR / "sql" / "claims_ahj.sql", "r") as file:
        query = file.read()
...
def read_data():
    ...
    """
    try:
        engine = get_conn_engine(read_passcode)
        ...
        try:
            engine = get_conn_engine(read_passcode)
            return pd.read_sql_query(query, engine).head()
        except Exception as e:
            print(f"Second attempt failed with error: {str(e)}")
            print("Reading from live data")
            try:
                engine = get_conn_engine(live_passcode)
                return pd.read_sql_query(query, engine)
            except Exception as e:
                print(f"Reading from live data attempt failed with error: {str(e)}")
                pass
...
def dev_response(info, services, model="accounts/fireworks/models/deepseek-v3"):
    ...
    """
    try:
        json_model = ChatFireworks(model=model, temperature=0.0, max_tokens=8000, model_kwargs={"top_k": 1}).bind(
...
def make_preds(df):
    ...
    with open('responses.json', 'w') as f:  # dev
...
def write_preds(responses, df):
    ...
    pred_df = df[["VisitServiceID", "Medical_Prediction", reason_col]]
    ...
    try:
        ...
        new_df.to_csv(BASE_DIR / 'etl_analysis.csv', mode='a', header=False, index=False)
    except Exception as e:
        pass
