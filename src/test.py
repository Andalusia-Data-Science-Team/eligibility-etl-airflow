...
def update_table(table_name, df, retries=5, delay=30):
    ...
    # Build connection parameters string
    ...
    # Create SQLAlchemy engine using odbc_connect format
    ...
    if "insertion_date" in df_clean.columns:
        df_clean["insertion_date"] = pd.to_datetime(df_clean["insertion_date"])
    ...
    while attempt < retries:
        try:
            ...
            df_clean.to_sql(
                ...
                index=False,
                chunksize=1000,
                method="multi",  # Use multi-row INSERT statements for better performance
            )

            logger.info(
                f"Table {table_name} updated successfully with {len(df_clean)} records"
            )
            return

        except SQLAlchemyError as e:
            attempt += 1
            logger.error(f"SQLAlchemy error - Attempt {attempt} failed: {e}")
            ...
            attempt += 1
            logger.error(f"General error - Attempt {attempt} failed: {e}")
...
def Beneficiary_api(patient_iqama_id):
    # Define the API URL
    url = (
        f"http://10.111.111.6:8000/check-insurance?patient_iqama_id={patient_iqama_id}"
    )
    # Send a GET request
