...
def next_hourly_slots(slots_24h: list[int]) -> tuple[datetime, datetime]:
    ...
    now = _now()
    today_slots = [now.replace(hour=h, minute=0, second=0, microsecond=0) for h in slots_24h]
    future = [t for t in today_slots if t > now]
    prev = max([t for t in today_slots if t <= now], default=now.replace(hour=0, minute=0, second=0, microsecond=0))
    nxt = future[0] if future else now.add(days=1).replace(hour=slots_24h[0], minute=0, second=0, microsecond=0)
    return (prev, nxt)
