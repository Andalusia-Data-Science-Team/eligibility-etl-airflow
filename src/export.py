...
def render_docx(
    df: pd.DataFrame, visit_id: str, patient_hint: str | None = None
) -> bytes:
    """
    ...
    if not df.empty:
        vdate = (
            str(df["VisitStartDate"].iloc[0]) if "VisitStartDate" in df.columns else "-"
        )
        payer = (
            str(df["ContractorEnName"].iloc[0])
            if "ContractorEnName" in df.columns
            else "-"
        )
