...
CASE 
    ...
    ELSE 'U' -- Default to 'Unknown' if no match
END AS marital_char,

--CASE WHEN P.NationalityID=56 THEN 'NI'
--WHEN P.NationalityID !=56 AND P.IdentificationTypeID =2 THEN 'PPN'
-- ELSE 'PRC' END 
 CASE  P.IdentificationTypeID WHEN 2 THEN 'NI'
WHEN 3 THEN 'PPN'
WHEN 5 THEN 'PRC'
WHEN 8 THEN 'BORD'
ELSE 'VISA' END AS nationality,
 IDY.ENNAME,
...
LEFT JOIN Billing.Contractor BC
ON BC.ID=vfi.ContractorID 
LEFT JOIN MPI.LKP_IdentificationType IDY
ON IDY.ID=P.IdentificationTypeID
INNER JOIN Billing.ContractorGateWayMappings CGWM
ON CGWM.ContractorID=ISNULL(BC.ParentID,BC.ID ) AND CGWM.GateWayID=3
