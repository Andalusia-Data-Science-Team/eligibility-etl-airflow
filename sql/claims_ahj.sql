WITH
    VISIT_CTE AS (
        ...
        WHERE VS.ClaimDate >= DATEADD(HOUR, -1, GETDATE())
          AND VS.IsDeleted = 0
    ...
    ChiefComplaint_CTE AS (
        ...
        WHERE ISNULL(CC.UpdatedDate, CC.CreatedDate) >= DATEADD(HOUR, -1, GETDATE())
          AND CC.IsDeleted = 0
    ),
    Diagnose_CTE AS (
        SELECT DISTINCT VS.ID
        ...
        WHERE (ISNULL(PCD.UpdatedDate, PCD.CreatedDate) >= DATEADD(HOUR, -1, GETDATE())
               OR ISNULL(PC.UpdatedDate, PC.CreatedDate) >= DATEADD(HOUR, -1, GETDATE()))
          AND PC.IsDeleted = 0
          AND PCD.IsDeleted = 0
    ),
    Symptom_CTE AS (
